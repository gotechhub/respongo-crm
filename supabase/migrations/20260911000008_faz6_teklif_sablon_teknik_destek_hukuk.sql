...
technical_shared(items_tr, items_en) as (
  values (
    '["Bulut tabanlı, kurulum gerektirmeyen erişim — güncel bir web tarayıcısı yeterlidir","Aktarımda ve beklemede endüstri standardı şifreleme","Bilgi güvenliği yönetiminde ISO 27001 çerçevesiyle uyumlu kontroller, SOC 2 Type II ilkelerine uygun bağımsız denetim yaklaşımı","KVKK ve GDPR gereksinimlerine uygun veri işleme yaklaşımı; talep halinde ayrı bir Veri İşleme Sözleşmesi (DPA) imzalanabilir","Rol tabanlı yetkilendirme, en az ayrıcalık ilkesi ve çok faktörlü kimlik doğrulama (MFA)","Periyodik zafiyet taramaları ve bağımsız sızma testleri (VAPT)","Düzenli yedekleme ve geri dönüş testleri; kurtarma süresi/nokta hedefleri (RTO/RPO) hizmet sözleşmesinde belirtilir","SSO, HRIS, Microsoft Teams ve Zoom ile hazır entegrasyonlar; açık API ve webhook desteği","SCORM ve xAPI içerik standartlarıyla tam uyumluluk","22''den fazla dilde içerik ve arayüz desteği","Çevrimdışı çalışabilen mobil öğrenme deneyimi"]'::jsonb,
    '["Cloud-based, no-install access — a current web browser is sufficient","Industry-standard encryption in transit and at rest","Information security controls aligned with the ISO 27001 framework, following SOC 2 Type II independent-audit principles","Data processing aligned with KVKK and GDPR requirements; a separate Data Processing Agreement (DPA) can be executed on request","Role-based authorization, least-privilege principle and multi-factor authentication (MFA)","Periodic vulnerability scans and independent penetration testing (VAPT)","Regular backups and recovery testing; recovery time/point objectives (RTO/RPO) are defined in the service agreement","Ready-made SSO, HRIS, Microsoft Teams and Zoom integrations; open API and webhook support","Full compliance with SCORM and xAPI content standards","Content and interface support in 22+ languages","Offline-capable mobile learning experience"]'::jsonb
  )
),
...
cross join lateral (
  values
    ('technical_specs', 41, 'Teknik Özellikler ve Güvenlik', 'Technical Specifications & Security',
      jsonb_build_object('items_tr', ts.items_tr, 'items_en', ts.items_en)),
